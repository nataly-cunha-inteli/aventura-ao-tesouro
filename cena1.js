class cena1 extends Phaser.Scene {
 ...
 create () {
    ...
    this.fundo = this.add.image(0, 0, 'fundo').setOrigin(0, 0).setScale(0.5);
    ...
    this.plataformaP = this.physics.add.staticImage(220, 400, 'plataforma_p').setScale(0.5).refreshBody();

    // cria de um grupo para as plataformas, também estático para que a personagem possa colidir com elas
    this.plataformas = this.physics.add.staticGroup(); 
    
    //adiciono cada plataforma ao grupo 'plataformas'. o método refreshBody() serve para atualizar suas dimensões, para que correspondam perfeitamente à imagem. Outros objetos do jogo também utilizam esse método
    this.plataformas.create(1250, 500, 'plataforma_m').setScale(0.5).refreshBody(); 
    this.plataformas.create(950, 250, 'plataforma_m').setScale(0.5).refreshBody();
    ...
    // criação do baú
    this.bau = this.physics.add.sprite(920, 100, 'bau').setScale(0.9).setDepth(9);
    ...
    function atualizarMoedasColetadas() {
        ...
        //adicionei um for que sempre irá atualizar a pontuação
    ...
    update () {
        ...
        if (this.teclado.left.isDown) {
            ...
            // ações para o caso da tecla para cima e a tecla para a direita serem clicadas ao mesmo tempo
            ...
            this.player.setVelocityY(-300); //define pulo
